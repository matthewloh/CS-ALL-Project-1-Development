...
try:
  ...
  subprocess.check_call(['py', '-m',' pip', 'install', 'pillow'])
  print('Done.')
  from PIL import ImageTk, Image, ImageOps
try:
...
class Window(Tk):
    def __init__(self, *args, **kwargs):
        ...
        # Left Container
        self.primaryframe = Frame(self, bg=LAVENDER)
        self.primaryframe.grid_propagate(0)
        self.singlecontainer()
        self.primaryframe.grid_rowconfigure(0, weight=0)
        self.primaryframe.grid_columnconfigure(0, weight=0)
        ...
        for F in (BlankFrame, RegistrationPage, LoginPage, MainPage2, EventView, EventRegistration, EventCreation, ViewParticipants, Calendar, FeedbackForm):
            frame = F(self.primaryframe, self)
            self.frames[F] = frame
    ...
    def onelargeonesmallcont(self): #used in sign up page, login page, event list, event registration
        self.primaryframe.grid(row=3, column=2, columnspan=13,
                               rowspan=12, sticky=N+S+E+W)

    def doublelargecontainers(self): #used in event list, event registration
        self.primaryframe.grid(row=2, column=2, columnspan=14,
                               rowspan=14, sticky=N+S+E+W)

    def singlecontainer(self):
        self.primaryframe.grid(row=2, column=2, rowspan=14,
...
class BlankFrame(Frame):
    def __init__(self, parent, controller):
        ...
        self.configure(bg=LIGHTPURPLE)
        self.grid(row=0, column=0, rowspan=20, columnspan=40, sticky=N+S+E+W)
        

        for x in range(14):
            self.columnconfigure(x, weight=1, uniform='x')
            Label(self, height=10, bg=LAVENDER, borderwidth=1, relief="solid").grid(
                row=0, column=x, rowspan=1, columnspan=2, sticky=N+S+E+W)
        for y in range(7):
            self.rowconfigure(y, weight=1, uniform='x')
            Label(self, height=13, bg=LAVENDER, bd=1, borderwidth=1, relief="solid").grid(
                row=y, column=0, rowspan=2, columnspan=1, sticky=N+S+E+W)
        # blanklabel = Label(self, text="blank",height=1,width=1,bg=DARKBLUE)
        # blanklabel.grid(row=7, column=13, rowspan=1, columnspan=1, sticky=N+S+E+W,pady=5)
        # blanklabel.grid_propagate(0)
