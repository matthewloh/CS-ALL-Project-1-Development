...
class Window(Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.conn = sqlite3.connect('dummy.db')
    ...
    def welcomelabel(self, name, role):
        ...
        self.namelabel = Button(self.welcomeframe, width=1, height=1, text="",font=("Atkinson Hyperlegible", 30), fg="white",bg=DARKBLUE, command=lambda:self.make_a_container())
    ...
    def make_a_container(self):
        ...
        self.randomframe.grid(row=10, column=20, rowspan=6, columnspan=12,
                            sticky=NSEW)
    def createcalendarframe(self):
        self.randomframe = Frame(self, bg=OTHERPINK, width=1, height=1,
                                    borderwidth=1, relief="flat")
        self.randomframe.grid_propagate(False)
        for x in range(12):
            self.randomframe.columnconfigure(x, weight=1, uniform='row')
            Label(self.randomframe, width=1, bg=OTHERPINK).grid(
                row=0, column=x, sticky=NSEW)
        for y in range(12):
...
class MainPage(Frame):
    def __init__(self, parent, controller):
        ...
        feedbackbutton = Button(self, image=self.feedbackimage, width=1, height=1, relief="flat", fg='#000000', bg='#FFF5E4',
        command=lambda: [
        ...
        'Lucida Calligraphy', 16), width=1, height=1, relief="flat", fg='#000000', bg='#FFF5E4',  command=lambda:
        ...
        calendarbutton = Button(self, text="Calendar", font=(
            'Lucida Calligraphy', 16), width=1, height=1,  relief="flat", fg='#000000', bg='#FFF5E4', command=
            lambda: [
    ...
    def update_eventnames(self):
        ...
        with self.conn:
            self.c.execute("SELECT COUNT(event_name) FROM eventcreation")
        self.eventcount = self.c.fetchone()[0]
        if self.eventcount == 0: #if there are no events, just make it say because index is already -1 and -2
            self.firsteventnamebutton.config(text="No Events, refresh to check again")
            self.secondeventsnamebutton.config(text="No Events, refresh to check again")
            return
        elif self.eventcount == 1: #the top button will have have to pass index 0
        ...
        self.eventnames = self.c.fetchall()
        self.twoeventnames = [i[0] for i in self.eventnames]
        print(self.twoeventnames)
        self.firsteventnamebutton.config(text=self.twoeventnames[0])
        self.secondeventsnamebutton.config(text=self.twoeventnames[1])
    def loadtheeventinquestion(self, index):
        if index <= -1:
            messagebox.showinfo("No Events", "There are no events to view, please refresh if you expect there to be events")
            return
        eventviewrfrnce = self.controller.get_page(EventView)
        eventviewrfrnce.load_event(index)
        self.controller.show_frame(EventView)
        self.controller.togglebuttonrelief(self.controller.eventlistbutton)
...
class EventView(Frame):
    def __init__(self, parent, controller):
        ...
        self.getregisteredbutton = Button(self, image=self.getregisteredbuttonimage, width=1, height=1, bg=LIGHTPURPLE, command=lambda:
...
class EventRegistration(Frame):
    def __init__(self, parent, controller):
        ...
        # generate a comboboxselected event when the user clicks on the dropdown
...
class ManagementSuite(Frame):
    ...
    def manageeventsframe(self):
        ...
        # self.criteriasearchbutton = Button(self.interfaceframe, image=self.criteriasearchimage, width=1, height=1, bg=LIGHTPURPLE, relief=FLAT, command=lambda:self.search_by_criteria())
    ...
    def database_queries(self):
        ...
        if self.eventcount == 0:
            ...
            try:
                ...
                for widget in self.rightframe.winfo_children():
                    if widget != self.rightframebackgroundlabel:
                        widget.destroy()
            except:
                pass
            return
        ...
        self.overallframes[0].grid()
            ...
            #second page
        #     elif indexofeventdetails < 8: # 4-7 
        ...
        #     # #third page
        #     elif indexofeventdetails < 12: # 8-11
        ...
        # Label(self.overallframes[3], text=f"{event_name}", image=self.labelbackground, width=1, height=1, font=("Avenir Next Bold", 18),fg="white", bg=LIGHTPURPLE, compound=CENTER).grid(row=indexofeventdetails*3, column=0, rowspan=2, columnspan=12, sticky=NSEW)
        # Label(self.overallframes[4], text=f"{event_name}", image=self.labelbackground, width=1, height=1, font=("Avenir Next Bold", 18),fg="white", bg=LIGHTPURPLE, compound=CENTER).grid(row=indexofeventdetails*3, column=0, rowspan=2, columnspan=12, sticky=NSEW)
    ...
    def delete_event(self, event_name, event_key):
        ...
        resetdropdown.eventdropdown.current(0)
    ...
    def read_student_details(self, eventname, studentname, index):
        ...
        self.currentlyeditinglabel = Button(self.frametoshowdetails[index], state=DISABLED, text=f"You are currently editing the student, {self.fullname}", font=("Avenir Next", 12), fg=BLACK, bg=WHITE, width=1, height=1)
        ...
        self.eventnamelabel = Label(self.frametoshowdetails[index], text=f"The details for student {self.fullname} in the event: {eventname}", font=("Avenir Next Medium", 16), fg=BLACK, bg=WHITE, width=1, height=1)
    ...
    def generate_studentlist(self, information:tuple):
        ...
        if self.countofstudentsregistered == 0:
            ...
            self.nostudentsregisteredlabel = Label(self.frametoshowdetailsevent, text=f"No students registered\n for this event\n{self.nameofevent}", font=("Avenir Next", 18), width=1, height=1, justify=CENTER, anchor=CENTER, fg=BLACK, bg=OTHERPINK)
